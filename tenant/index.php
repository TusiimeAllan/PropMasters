...
if (isset($_GET['property_id'])) {
    ...
    $updateQuery = "UPDATE properties SET likes = likes + 1 WHERE id = $propertyId";
        ...
        <div class="row">
            <div class="col-md-12 d-flex flex-column justify-content-center align-items-center">
                <div class="mt-3">
                    ...
                    $property_query = "SELECT * FROM properties WHERE approved=1";
                    $property_result = mysqli_query($conn, $property_query);

                    if (mysqli_num_rows($property_result) == 0) {
                        // No properties found
                        echo "No properties on the System yet.";
                    } else {
                        while ($row = mysqli_fetch_assoc($property_result)) {
                            ...
                            echo '<div class="mt-3">';
                            echo '<a href="index.php?property_id=' . $row['id'] . '" class="btn btn-primary mr-2">Like</a>';
                            echo '</div>';
                            echo '</div>';
                            echo '</div>';
                            echo '</div>';
